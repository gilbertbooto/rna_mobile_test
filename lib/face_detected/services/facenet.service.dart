...
import 'package:rna_mobile_test/face_detected/services/image_converter.dart';
...
class FaceNetService {
  ...
  List? predict2() {
    /// search closer user prediction if exists
    return _searchResult2(this._predictedData);
  }
  ...
  String? _searchResult(List predictedData) {
    ...
    //print('ggggggggggggggggggggggggggg $predRes');
    return predRes;
  }

  List? _searchResult2(List predictedData) {
    Map<String, dynamic> data = _dataBaseService.db;

    print('Je charge les2 $data');
    ...
    for (String label in data.keys) {
      ...
      if (currDist <= threshold && currDist < minDist) {
        minDist = currDist;
        prediata = predictedData;
      }
    }
    // print('ggggggggggggggggggggggggggg22 $prediata');
    return prediata;
  }
